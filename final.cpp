...
void cloud2()
{
    if (cloudStatus == 1)
    {
        cloud_X2 += 0.3; // speed 3x 
    }
    if (cloud_X2 > 600)
    {
        cloud_X2 = -1150; // if goes out from the screen pixel then start againg drawing clouds....
    }
    glPushMatrix();
    glTranslatef(cloud_X2 + 500, cloud_Y2 + 50, 0); // gaps b/w cloud in horizontal or vertical manner...
    drawCloud();
    glPopMatrix();
}
